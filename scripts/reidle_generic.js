// Rebuild idle/east(direct) + idle/west(mirror) from a pure side-profile idle source.
// Same fitting approach as rewalk_generic (tf from east rotation bbox), keeps all frames.
// args: <charId> <eastRotationPng> <sideSrcDir>
const { createCanvas, loadImage } = require('@napi-rs/canvas');
const fs=require('fs'),path=require('path');
const OUTC=process.argv[2], ROT=process.argv[3], SRC=process.argv[4];
const OUT=`/home/user/Maodou/art/05_pixellab/${OUTC}_field`;
...
(async()=>{
  ...
  let n=0; while(fs.existsSync(ff(SRC,n))) n++;
  for(const [dir,mir] of [['east',0],['west',1]]){
    ...
    for(let i=0;i<n;i++){const f=await loadImage(ff(SRC,i));fs.writeFileSync(path.join(d,String(i).padStart(2,'0')+'.png'),place(f,t,mir).toBuffer('image/png'));}
  }
  const man=JSON.parse(fs.readFileSync(path.join(OUT,'manifest.json'),'utf8'));
  man.anims.idle.frames.east=n; man.anims.idle.frames.west=n;
  fs.writeFileSync(path.join(OUT,'manifest.json'),JSON.stringify(man,null,1));
  console.log(OUTC,'idle east+west rebuilt from side-profile,',n,'frames');
})();
