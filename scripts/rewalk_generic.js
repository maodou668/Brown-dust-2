...
(async()=>{
  ...
  let n=0; while(fs.existsSync(ff(SRC,n))) n++; const out=n-1;
  for(const [dir,mir] of [['east',0],['west',1]]){
    ...
    for(let i=1;i<n;i++){const f=await loadImage(ff(SRC,i));fs.writeFileSync(path.join(d,String(i-1).padStart(2,'0')+'.png'),place(f,t,mir).toBuffer('image/png'));}
  }
  ...
  console.log(OUTC,'run east+west rebuilt from side-profile,',out,'frames');
})();
