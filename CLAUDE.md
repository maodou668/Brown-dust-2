...
- **🛑 做事协议（铁律 · 每个任务都照走，用户明确要求写入）**：
  ...
  6. **🎨 美术资产强制闸门 —— 接入前必跑一条命令并贴输出（强制函数，禁止"我看着行")**：
     - **唯一入口**：接入任何角色/服装 sprite 前，**必须运行 `node scripts/artgate.js <char_field前缀>` 并把它的原始输出（全向脚漂移数字 + PASS/FAIL）贴给用户**。**没有这份输出 = 没做闸门 = 不接入**；FAIL 直接 exit 1 挡住。跑完还要打开它生成的 `_artgate_idle/run.png` **逐向目视朝向**（尤其**背向斜角 NE/NW/N/E 最易抽腿**）。抽腿按管线 re-roll≤2 或 `synthnwne.js`/`synthidle.js` 合成呼吸兜底。
     - **walk/run**：**铁律——必须先 `create_character_state` 建 mid-stride walk-state，再在其上 `walking_cycle`**，绝不直接做 run；组装后必跑 `scripts/rundiag.js` 走路朝向闸门。
     - **母图**：出 8 向后先过母图姿势闸门（east/west 正侧面）再做任何动画。
     - 一句话：**管线里写了闸门脚本的，必须跑脚本+贴数字+全向图，不许"我看着行"。**
  - 违反此协议 = 返工。今日返工根因就是：不确认理解、做完不核对、不自测就汇报、**拿一个方向的证据判全向通过、忘了 walk 先建 state**。
