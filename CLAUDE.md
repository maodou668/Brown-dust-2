...
- ⚠️ **成功路径(已由 Lecliss 一次过验证，做任何角色前读 ART_PIPELINE.md)**：
  ...
  ③ idle 用 **north-west 源**(gnorm IMIR 镜像出 NE，防抖腿)；**走路直接在母图上做 walking_cycle**(母图朝向已过姿势闸门=正确)，**不要再单独建 mid-stride walk-state**——它重生成各向旋转会把斜角朝向画反(Teried 东接西/SE接SW 踩坑)；东西方向另出 `walk_side` 纯侧面经 `rewalk_generic` 覆盖(防 3/4)；施法描述**直接写技能内容**(自带火焰/奥术 VFX)。⚠️ 实测母图直接出的走路脚漂移更小(Lia 2.0px < 老 walk-state 的 Helena 5.3px)，不是"漂移"来源。
