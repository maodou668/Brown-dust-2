...
- **🛑 做事协议（铁律 · 每个任务都照走，用户明确要求写入）**：
  ...
  6. **🎨 美术资产专项闸门（禁止用单方向/局部证据判"全向过")**：
     - **idle**：必跑 `scripts/fd8.js` 打出**全 8 向脚漂移数字**并贴出来；再**逐向目视**（尤其**背向斜角 NE/NW/N/E 最易抽腿**，管线明写）。有一向没看/没跑 = 不算过；抽腿按管线走 re-roll≤2 或 `synthnwne.js`/`synthidle.js` 合成呼吸兜底。
