...
const BattleUI = {
  ...
  drawBattleSprites(now) {
    ...
    this.root.querySelectorAll('canvas.u-sprite').forEach(cv => {
      ...
      const dir = c.side === 'enemy' ? 'south' : 'north';   // 敌方角色单位面向镜头
      ...
      const arr = (sp.imgs[anim] && sp.imgs[anim][dir] && sp.imgs[anim][dir].length) ? sp.imgs[anim][dir] : sp.imgs.idle[dir];
  ...
  SKILL_VFX: {
    ...
    oath_aegis:       { castMs: 360, telegraphMs: 220, burst: 'guard_barrier', tint: '#ffd35a' },
    ...
    mending_song:     { castMs: 380, telegraphMs: 240, burst: 'heal_bloom', tint: '#ffe7a0' },
    holy_smite:       { castMs: 340, telegraphMs: 200, burst: 'dawn_slash', tint: '#fff0b0' },
    tidal_burst:      { castMs: 380, telegraphMs: 300, burst: 'water_vortex', tint: '#5a9fff' },
    shield_bash:      { castMs: 320, telegraphMs: 180, burst: 'cleave_slash', tint: '#c9a05a' },
  ...
  unitHtml(c, side, pos, idx, count) {
    ...
    // 角色型单位（我方 & char 型敌人如剧情缉捕兵）都走序列帧；纯怪物才用怪物图/占位
    const isCharUnit = !!window.GameData.CHARACTERS[c.charId];
    const charDef = isCharUnit ? window.GameData.CHARACTERS[c.charId] : window.GameData.ENEMIES[c.charId];
    const fallback = isCharUnit ? window.GameData.CLASSES[charDef.cls].icon : '👹';
    const icon = (isCharUnit && this.FIELD_SPRITE[this.spriteKey(c)])
