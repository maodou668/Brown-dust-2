...
| 2c | **走路方向接反**(东接西/SE接SW) + 东西带角度 | 额外的 mid-stride **walk-state 重新生成各向旋转时把斜角朝向画反**(SE 画成朝 SW);且 walking_cycle 东西仍会漂成 3/4 | **直接在母图上生成 walking_cycle**(母图 8 向朝向已过姿势闸门=正确), 别用单独 walk-state;东西另出 `walk_side` 纯侧面经 `rewalk_generic` 覆盖 | **走路朝向闸门**:组装后渲染 run 8 向与 idle 8 向逐列对照,**每个方向 run 必须和 idle 同朝向**;不一致就是接反/源朝向错 |
